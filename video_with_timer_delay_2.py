...
while(True):
    ...
    for (column, row, width, height) in mask_cv_cascade:
        ...
        if(tick==2):
            ...
            label_res = getLabel(res[0])
            cv2.putText(frame, str(label_res), (200, 250), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 255))
            temp = label_res
        else:
            cv2.putText(frame, str(temp), (200, 250), cv2.Font, 1, (0, 0, 255))
